...
function Mypage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch("https://maketerbackend.fly.dev/api/v1/profile");
    ...
    <MainContainer>
      <DeviceFrameset device="iPad Mini">
        <Container>
          ...
          <TabContainer>
            <Tab isActive={selectedTab === "posts"} onClick={() => setSelectedTab("posts")}>
              <FaTh />
            </Tab>
            <Tab isActive={selectedTab === "favorites"} onClick={() => setSelectedTab("favorites")}>
              <FaStar />
            </Tab>
          </TabContainer>
          {selectedTab === "posts" ? (
